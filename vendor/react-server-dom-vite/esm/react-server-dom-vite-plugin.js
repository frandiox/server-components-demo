  ...
  return {
    ...
    transform: function (code, id) {
      ...
      if (/\/react-server-dom-vite.js/.test(id)) {
        ...
        var userGlob = path.join(importerToRootPath, 'src', CLIENT_COMPONENT_GLOB);
        ...
        var injectedGlobs = "Object.assign(Object.create(null), " + importers.map(function (_ref3) {
          var glob = _ref3[0],
              prefix = _ref3[1];
          return "__vncp(import.meta.glob('" + glob + "'), '" + prefix + "')";
        }).join(', ') + ");";
        return code.replace(/\{\s*__INJECTED_CLIENT_IMPORTERS__[:\s]*null[,\s]*\}\s*;/, injectedGlobs + serializedNormalizePaths());
      }
    }
  };
}
...
async function proxyClientComponent(id, src) {
  ...
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      ...
      if (key !== DEFAULT_EXPORT && /^use[A-Z]|Fragment$|Context$|^[A-Z_]+$/.test(key)) {
  ...
  componentExports.forEach(function (key) {
    ...
    var componentName = isDefault ? id.split('/').pop().split('.').shift() : key;
    proxyCode += "export " + (isDefault ? DEFAULT_EXPORT : "const " + componentName + " =") + " wrapInClientProxy({ name: '" + componentName + "', id: '" + id + "', component: allImports['" + key + "'], named: " + String(!isDefault) + " });\n";
  });
  return proxyCode;
}
